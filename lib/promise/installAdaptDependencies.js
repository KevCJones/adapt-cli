...
module.exports = function installAdaptDependencies (properties /* { renderer, localDir } */) {
    ...
    installCommand.install(properties.renderer, Constants.Registry, properties.localDir, function (err) {
        if(err) deferred.reject(err);
        
        process.chdir(cwd);
        deferred.resolve(properties);
    }).then(
    installCommand.install(properties.renderer, Constants.RegistryAlt, properties.localDir, function (err) {
        if(err) deferred.reject(err);
